...
/// <summary>
///     Extends how ScriptableObject object references are displayed in the
///     inspector
///     Shows you all values under the object reference
///     Also provides a button to create a new ScriptableObject if property is
///     null.
/// </summary>
...
public class ScriptableObjectPropertyDrawer : PropertyDrawer
{
    ...
    private static readonly List<string> _ignoreClassFullNames = new()
    {
        "TMPro.TMP_FontAsset",
    };
    ...
    )
    {
        ...
        totalHeight += EditorGUIUtility.standardVerticalSpacing * 8;
    ...
    //         // BindProperty(property.objectReferenceValue);
    ...
    //         if (property.isExpanded)
    //         {
    //             // Draw a background that shows us clearly which fields are part
    ...
    //             {
    //                 MonoScript ms = MonoScript.FromMonoBehaviour(
    //                     (MonoBehaviour)property.serializedObject.targetObject
    //                 );
    //                 selectedAssetPath = System.IO.Path.GetDirectoryName(
    //                     AssetDatabase.GetAssetPath(ms)
    //                 );
    //             }
        ...
        else
        {
            if (GUI.Button(buttonRect, "+"))
            {
                ...
                if (property.serializedObject.targetObject is MonoBehaviour)
                {
                    ...
                    selectedAssetPath = Path.GetDirectoryName(
                        AssetDatabase.GetAssetPath(ms)
                    );
                }
    ...
    ) where T : ScriptableObject
    {
        ...
        else
        {
            ...
            // but both code paths seem to need to be a foldout or
            // the object field control goes weird when the codepath changes.
            // I guess because foldout is an interactable control of its own and
    ...
    private static ScriptableObject CreateAssetWithSavePrompt(
