    ...
    IPointerUpHandler
{
    public float Horizontal =>
        snapX ? SnapFloat(input.x, AxisOptions.Horizontal) : input.x;
    public float Vertical =>
        snapY ? SnapFloat(input.y, AxisOptions.Vertical) : input.y;
    ...
    public void OnDrag(PointerEventData eventData)
    {
        ...
        var position =
            RectTransformUtility.WorldToScreenPoint(cam, background.position);
    ...
    private void FormatInput()
    {
        ...
        {
            input = new(input.x, 0f);
        }
        else if (axisOptions == AxisOptions.Vertical)
        {
            input = new(0f, input.y);
        }
    }
